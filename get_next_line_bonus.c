...
/*   Updated: 2021/06/10 06:25:13 by prolling         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
* return 1: valid FD
*/
static size_t	ft_valid_fd(int fd)
{
	if (fd < 0 || fd > MAX_FD)
		return (0);
	else
...
*/
static int	ft_reload_buf(int fd, char *buf, size_t start, size_t end)
{
	int	nb_read;

	nb_read = read(fd, (void *)&buf[start], end - start - 1);
	return (ft_strclen((const char *)buf, '\0') + nb_read + start);
}
...
*/
static void	ft_append_buf_line(char **line, char *buf, int c)
{
	char	*newline;
	char	*m;
	size_t	ll;
	...
	newline = (char *)ft_calloc(sizeof(char), (ll + bl + 1));
	if (!newline)
		return ;
	m = newline;
	c = 0;
...
*/
static size_t	ft_shift_buf(char *buf, char *nl_pos)
{
...
int	get_next_line(int fd, char **line)
{
	...
	*line = (char *)ft_calloc(sizeof(char), 1);
	...
	while (ft_strchr((const char *)fd_buf[fd], '\n') == 0)
	{
		...
		buf_state = ft_reload_buf(fd, fd_buf[fd], 0, BUFFER_SIZE + 1);
		if (!buf_state)
			return (0);
	}
	...
	buf_state = ft_reload_buf(fd, fd_buf[fd], shifted, BUFFER_SIZE + 1);
	return (1);
}
